...
contract('StandardToken', function (accounts) {
    ...
    beforeEach(async function () {
        token = await StandardTokenMock.new();
    ...
    it('should throw an error when trying to transfer more than balance', async function () {
        let token = await StandardTokenMock.new();
        await token.mint(accounts[0], 100);
        await expectThrow(token.transfer(accounts[1], 101));
    });
    ...
    describe('validating allowance updates to spender', function () {
        ...
        it('should increase by 50 then decrease by 10', async function () {
            ...
            assert.equal(+preApproved.plus(50), postIncrease);
            await token.decreaseApproval(accounts[1], 10);
            let postDecrease = await token.allowance(accounts[0], accounts[1]);
            assert.equal(+postIncrease.minus(10), postDecrease);
        });
    });
